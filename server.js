...
const { SNAPSHOT } = require('./prompts/systemPrompt');
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && req.url === '/webhook') {
    try {
      ...
      const MACRO_HEADER = '\n\nАКТУАЛЬНЫЕ ДАННЫЕ (фактический фон; при конфликте с документом — документ важнее):\n';
      const systemPrompt = useTajik
        ? tajik.getTajikSystemPrompt() + MACRO_HEADER + SNAPSHOT +
          (trimmedText ? `\n\nМАЪЛУМОТИ ИЛОВА: ${tajik.normalizeTajik(trimmedText.slice(0, 500))}` : '')
        : `ВАЖНО: Готовые фактические цифры (ВВП, инфляция, курс, экспорт и т.д.) бери ТОЛЬКО из загруженного документа или из блока АКТУАЛЬНЫЕ ДАННЫЕ ниже. НИКОГДА не выдумывай числа и не подставляй их в формулы сам. Формулы применяй лишь когда нужно вычислить показатель из данных, которые реально присутствуют. Если точной цифры нет ни в документе, ни в снимке — так и скажи.
...
- Отвечай простым текстом на русском языке, краткими абзацами. НЕ используй markdown-разметку: никаких #, ##, ###, |, **, таблиц из символов. Если нужна таблица — словами или простым перечислением.` + MACRO_HEADER + SNAPSHOT;
